...
class PriorityQueue:
    ...
    def update(self, message):
        ...
        # print("&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&")
        # print("update time:", datetime.datetime.now())
        # temple_tuple = ( message.getID(),tuple(message.getPriority()),message.getSender() )
        # print("message: ",message.getID(),tuple(message.getPriority()),message.getSender())
        # print("pid: ",os.getpid())
        # print(temple_tuple in received_repo)
        # print("PQ: ",end="")
        # self.printQ()
        # print("received_repo",received_repo)
        # print("position =",pos)
        # print("&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&")
        
        node1, time1 = message.getPriority()
        node2, time2 = self.q[pos].getPriority()
        if( node1==node2  and time1== time2 ):
            return
...
def get_events():
    ...
    while True:
        for content in sys.stdin:
            ...
            msg_tracker_lock.acquire()
            msg_tracker[msg.getID()].append(time.time())
            msg_tracker_lock.release()
...
def deliver(msg):
    ...
    if msg.getID() not in msgID_repo:
        ...
        # insert into PQ
        PQ.push(msg)
        # print("deliver time:", datetime.datetime.now())
        # print("ID:",os.getpid(),"After PQ.push: PQ: ", end="" )
        # PQ.printQ()
...
def multicast(msg):
    ...
    for nodeName in node_socket_dict:
        ...
        for _ in range(max_send_times):
            try:
                num_bytes = cur_socket.send(json_msg)  # is send fails ?
                num_bytes_send_lock.acquire()
                num_bytes_send += num_bytes
                num_bytes_send_lock.release()
                flag = 0  # send successfully
                # if(num_bytes == 0):
                    #print("ONE NODE: ",nodeName," FAIL")
                break
            except:  
                continue 
        ...
        if flag:
            ...
            cur_socket.close()
            dead_node_list.append(nodeName)
            num_connected_nodes -= 1
    #we need maintain dead_node_list becasue we can not del during interation
    for node_name in dead_node_list:
        del  node_socket_dict[node_name]
...
def deliver_queue_head():
    ...
    while True:
        ...
        # print()
        # print("deliver_queue_head time:", datetime.datetime.now())
        # print("PID: ",os.getpid(),"messageID:",message.getID(),"num_receive_node:",num_receive_node," num_connected_nodes+1: ", num_connected_nodes + 1 )
        # print(msgSender_dict[message.getID()])
        # print("PQ:",end="")
        # PQ.printQ()
        # print()
        #print("!!!!!!checking if we can pop PQ !!!!!!,num_connected_nodes + 1:" ,num_connected_nodes)
        #print("msgSender_dict[message.getID()]: ", msgSender_dict[message.getID()])
        if num_receive_node >= num_connected_nodes + 1: # 1 means itself, this is bacause when some node fails, we may already receieve these failed nodes reply
            ...
            #print(local_name,"msg_tracker[message.getID()]:",msg_tracker[message.getID()],message.getID())
            msg_tracker[message.getID()].append(time.time()) 
            
            stats = open('./stats/'+local_name + '_stats.csv', 'a')
            stats_writer = csv.writer(stats)
            #print([message.getID()]+ [msg_tracker[message.getID()][0]]+[msg_tracker[message.getID()][1]] +[num_bytes_send+num_bytes_recv])
            stats_writer.writerow([message.getID()]+ [msg_tracker[message.getID()][0]]+[msg_tracker[message.getID()][1]] +[num_bytes_send+num_bytes_recv])
...
def print_balances():
    ...
    for account in sorted(balance_dict):
        print(account + ": " + str(balance_dict[account]), end=" ")
    print()
...
def update_balances(queue_head_message):
    ...
    # resolve message
    # print(queue_head_message.getContent())
    ...
    if command == "DEPOSIT":
        ...
        # print("account, amount:",account, int(amount))
        ...
        # print("account, dest, amount",account, dest, int(amount))
        if account not in balance_dict:
            print("A TRANSFER transaction must use a source account that exists")
    ...
    while True:
        ...
        for data in data_list:
            ...
            if received_tuple in received_repo:
                ...
                # print("!!!!old received_tuple:",received_tuple)
                received_repo_lock.release()
                continue           
            else:
                # print("!!!!new received_tuple:",received_tuple)
            ...
            if cur_message.getID() not in msgID_repo:
                msg_tracker_lock.acquire()
                msg_tracker[cur_message.getID()].append(time.time())
                msg_tracker_lock.release()
